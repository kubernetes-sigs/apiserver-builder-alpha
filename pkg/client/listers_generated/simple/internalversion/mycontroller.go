...
type MyControllerLister interface {
	...
	List(selector labels.Selector) (ret []*simple.MyController, err error)
...
// List lists all MyControllers in the indexer.
func (s *myControllerLister) List(selector labels.Selector) (ret []*simple.MyController, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*simple.MyController))
	})
	return ret, err
}
...
type MyControllerNamespaceLister interface {
	...
	Get(name string) (*simple.MyController, error)
...
// List lists all MyControllers in the indexer for a given namespace.
func (s myControllerNamespaceLister) List(selector labels.Selector) (ret []*simple.MyController, err error) {
...
// Get retrieves the MyController from the indexer for a given namespace and name.
func (s myControllerNamespaceLister) Get(name string) (*simple.MyController, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(simple.Resource("mycontroller"), name)
	}
	return obj.(*simple.MyController), nil
}
