...
func main() {
	...
	vendorCmd.Flags().StringVar(&kubernetesVersion, "kubernetesVersion", "1.12.3", "version of kubernetes libs")
...
func BuildLocalVendor(tooldir string) {
	...
	RunCmd(c, "")
	os.MkdirAll(filepath.Join(tooldir, "src", "vendor", "sigs.k8s.io", "apiserver-builder-alpha"), 0700)
	c = exec.Command("cp", "-R", "-H",
		filepath.Join("pkg"),
