...
kubebuilder build docs --operations=true

# Use the swagger.json at docs/openapi-spec/swagger.json instead
# of getting it from a server.
kubebuilder build docs --build-openapi=false

# Use the server at my/bin/apiserver
kubebuilder build docs --server my/bin/apiserver
...
kubebuilder build docs --generate-toc=false
...
func RunDocs(cmd *cobra.Command, args []string) {
	...
	if buildOpenapi {
		...
		c := exec.Command("go", "build", "-o", filepath.Join("bin", "apiserver"), path)
