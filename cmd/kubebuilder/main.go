...
- build a docker container to install the API and controller into a namespace with RBAC configured:
  ...
  docker build -f Dockerfile.install . -t <install-image>
  docker build -f Dockerfile.controller . -t <controller-image>
  docker build -f Dockerfile.docs . -t <docs-image>
  kubectl run <name> --serviceaccount=<cluster-admin-account> \
      --image=<installer-image> --restart=OnFailure -- \
      ./installer --controller-image=<controller-image> --docs-image=<docs-image> \
