...
import (
	...
	. "sigs.k8s.io/apiserver-builder-alpha/example/basic/pkg/client/clientset_generated/clientset/typed/innsmouth/v1"
	...
	Describe("when listing a resource", func() {
		Context("using labels", func() {
			It("should find the matchings objects", func() {
				...
				By("returning the item for list requests")
				result, err := client.List(context.TODO(), metav1.ListOptions{LabelSelector: "foo=1"})
				Expect(err).ShouldNot(HaveOccurred())
				Expect(result.Items).To(HaveLen(1))
				Expect(result.Items[0].Spec).To(Equal(expected1.Spec))

				By("returning the item for list requests")
				result, err = client.List(context.TODO(), metav1.ListOptions{LabelSelector: "foo=2"})
				Expect(err).ShouldNot(HaveOccurred())
				Expect(result.Items).To(HaveLen(1))
